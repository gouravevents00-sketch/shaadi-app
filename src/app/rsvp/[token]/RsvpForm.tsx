...
import { Car, Plane, Train, Bus, Plus, X, CheckCircle2, ChevronDown, ChevronUp } from 'lucide-react'
...
export default function RsvpForm({
    ...
    return (
      <div className="min-h-screen bg-stone-50">
        ...
        <div className="max-w-lg mx-auto px-4 py-12 text-center">
          ...
          {attending ? (
            <>
              <h2 className="text-xl font-semibold text-stone-900 mb-2">We can't wait to see you!</h2>
            ...
            <>
              <h2 className="text-xl font-semibold text-stone-900 mb-2">We'll miss you!</h2>
  ...
  // ─── Details (combined optional step) ─────────────────────────
  if (step === 'details') {
    return (
      <div className="min-h-screen bg-stone-50">
        <Header />
        <div className="max-w-lg mx-auto px-4 py-6 space-y-4">
          {attending ? (
            <>
              <div>
                <p className="font-medium text-stone-900">
                  {attending ? 'Almost done! A few optional details' : 'Leave a message'}
                </p>
                <p className="text-stone-400 text-xs mt-0.5">
                  Skip anything you're not sure about — you can update later
                </p>
              </div>
              ...
              <div className="bg-white border border-stone-200 rounded-xl overflow-hidden">
                <button
                  ...
                  className="w-full flex items-center justify-between px-4 py-3 text-sm font-medium text-stone-700 hover:bg-stone-50"
                >
                  <span>✈️ Travel & arrival details</span>
                  <div className="flex items-center gap-2">
                    {(arrivalDate || travelMode) && (
                      <span className="text-xs text-emerald-600 font-normal">Filled</span>
                    )}
                    {showTravel ? <ChevronUp className="w-4 h-4 text-stone-400" /> : <ChevronDown className="w-4 h-4 text-stone-400" />}
                  </div>
                </button>
                {showTravel && (
                  <div className="px-4 pb-4 space-y-3 border-t border-stone-100 pt-3">
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        ...
                        <input type="date" value={arrivalDate} onChange={e => setArrivalDate(e.target.value)}
                          className="w-full border border-stone-200 rounded-lg px-3 py-2 text-sm text-stone-800 focus:outline-none focus:ring-2 focus:ring-rose-300 bg-white" />
                      </div>
                      <div className="space-y-1">
                    ...
                    <div>
                      <p className="text-xs font-medium text-stone-500 mb-2">How are you travelling?</p>
                      ...
                      <div className="space-y-3">
                        <div className="space-y-1">
                          ...
                          <input type="datetime-local" value={arrivalDatetime}
                            ...
                            className="w-full border border-stone-200 rounded-lg px-3 py-2 text-sm text-stone-800 focus:outline-none focus:ring-2 focus:ring-rose-300" />
                        </div>
                        {needsBookingRef && (
                          <input type="text" value={bookingRef} onChange={e => setBookingRef(e.target.value)}
                            placeholder={travelMode === 'flight' ? 'Flight no. / PNR (optional)' : 'Train no. / PNR (optional)'}
                            className="w-full border border-stone-200 rounded-lg px-3 py-2 text-sm text-stone-800 focus:outline-none focus:ring-2 focus:ring-rose-300" />
                        )}
                        <label className="flex items-center gap-2.5 cursor-pointer">
                          <input type="checkbox" checked={needsPickup} onChange={e => setNeedsPickup(e.target.checked)} className="rounded border-stone-300" />
                          <span className="text-sm text-stone-700">Request pickup from {travelMode === 'flight' ? 'airport' : 'station'}</span>
                        </label>
                      </div>
                    )}
                  </div>
              ...
              <div className="bg-white border border-stone-200 rounded-xl overflow-hidden">
                <button
                  ...
                  className="w-full flex items-center justify-between px-4 py-3 text-sm font-medium text-stone-700 hover:bg-stone-50"
                >
                  <span>👨‍👩‍👧 Who's joining you?</span>
                  <div className="flex items-center gap-2">
                    {family.length > 0 && (
                      <span className="text-xs text-emerald-600 font-normal">{family.length} added</span>
                    )}
                    {showFamily ? <ChevronUp className="w-4 h-4 text-stone-400" /> : <ChevronDown className="w-4 h-4 text-stone-400" />}
                  </div>
                </button>
                {showFamily && (
                  <div className="px-4 pb-4 border-t border-stone-100 pt-3 space-y-3">
                      ...
                      <div className="space-y-1.5">
                        {family.map((m, i) => (
                          <div key={i} className="flex items-center justify-between bg-stone-50 rounded-lg px-3 py-2">
                            ...
                            <button onClick={() => removeFamilyMember(i)} className="text-stone-400 hover:text-red-500">
                              <X className="w-4 h-4" />
                            </button>
                          </div>
                        ))}
                    ...
                    <button onClick={addFamilyMember} disabled={!newName.trim()}
                      className="w-full border border-stone-200 hover:bg-stone-50 disabled:opacity-40 text-stone-700 font-medium py-2 rounded-lg text-sm flex items-center justify-center gap-2">
            ...
            <>
              {/* Declined — just wishes + notes */}
              <div>
                <p className="font-medium text-stone-900">Leave a message 💌</p>
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button onClick={handleSubmit} disabled={loading}
              className={`flex-1 ${attending ? 'bg-rose-700 hover:bg-rose-800' : 'bg-stone-800 hover:bg-stone-900'} disabled:opacity-60 text-white font-medium py-3 rounded-xl text-sm transition-colors`}>
              {loading ? 'Confirming…' : attending ? 'Confirm RSVP ✓' : 'Submit'}
            </button>
          </div>
