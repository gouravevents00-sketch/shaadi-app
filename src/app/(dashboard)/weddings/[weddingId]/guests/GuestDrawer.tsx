...
import {
  Star, Phone, Mail, Users, CalendarDays, Plane, BedDouble, Utensils, StickyNote,
  MessageCircle, CheckCircle2,
} from 'lucide-react'
import { updateGuestField, toggleGuestEvent, upsertArrival, assignRoom } from './[guestId]/actions'
...
interface GuestData {
  ...
  family_group: string | null; checked_in_at: string | null
}

interface WeddingEvent { id: string; name: string; date: string; type: string }
  ...
  return (
    <div>
      ...
      <Input value={v} onChange={e => setV(e.target.value)} onBlur={commit}
        onKeyDown={e => e.key === 'Enter' && commit()} className="mt-1 h-9 text-sm" />
    </div>
  )
}

export default function GuestDrawer({
  open, onClose, weddingId, guest: initialGuest, events, guestEvents: initialGuestEvents,
  arrival: initialArrival, roomAlloc: initialRoomAlloc, rooms,
  onGuestUpdate,
}: {
  open: boolean
  onClose: () => void
  ...
  arrival: GuestArrival | null
  roomAlloc: GuestRoomAlloc | null
  rooms: GuestRoom[]
  onGuestUpdate?: (id: string, patch: Partial<GuestData>) => void
}) {
  const [tab, setTab] = useState<Tab>('profile')
  const [g, setG] = useState(initialGuest)
  const [ge, setGe] = useState(initialGuestEvents)
  const [alloc, setAlloc] = useState<GuestRoomAlloc | null>(initialRoomAlloc)
  const [saving, setSaving] = useState(false)

  // Sync when guest changes (drawer re-opened for different guest)
  // done by key prop on parent

  async function saveField(field: string, value: string | number | boolean | null) {
    setG(prev => ({ ...prev, [field]: value }))
    onGuestUpdate?.(g.id, { [field]: value } as Partial<GuestData>)
    const res = await updateGuestField(weddingId, g.id, { [field]: value })
    if ('error' in res) { toast.error(res.error); setG(initialGuest) }
    else toast.success('Saved')
  }

  function getGe(eventId: string) { return ge.find(x => x.event_id === eventId) }
  ...
  async function cycleRsvp(ev: WeddingEvent) {
    ...
    await toggleGuestEvent(weddingId, g.id, ev.id, next)
  }

  const [arrForm, setArrForm] = useState({
    mode: initialArrival?.mode ?? 'self',
    flight_train_no: initialArrival?.flight_train_no ?? '',
    arrival_time: initialArrival?.arrival_time ? initialArrival.arrival_time.slice(0, 16) : '',
    pickup_required: initialArrival?.pickup_required ?? false,
    status: initialArrival?.status ?? 'expected',
  })

  async function saveArrival() {
    setSaving(true)
    const res = await upsertArrival(weddingId, g.id, {
      mode: arrForm.mode,
  ...
  async function saveRoom() {
    ...
    setSaving(true)
    const res = await assignRoom(weddingId, g.id, roomForm.room_id, roomForm.check_in, roomForm.check_out)
    setSaving(false)
    if (res.error) toast.error(res.error)
    else {
      toast.success('Room assigned')
      const room = rooms.find(r => r.id === roomForm.room_id)
      setAlloc({ guest_id: g.id, room_id: roomForm.room_id, check_in: roomForm.check_in, check_out: roomForm.check_out, rooms: room ? { room_number: room.room_number, type: room.type, floor: room.floor } : null })
    }
  }

  async function removeRoom() {
    setSaving(true)
    await assignRoom(weddingId, g.id, '', '', '')
    setSaving(false)
    setAlloc(null)
  ...
  return (
    <Sheet open={open} onOpenChange={v => !v && onClose()}>
      <SheetContent className="w-full sm:max-w-lg overflow-y-auto p-0">
        {/* Header */}
        <div className="px-6 pt-6 pb-4 border-b border-stone-100 bg-white sticky top-0 z-10">
          <SheetHeader className="mb-0">
            <div className="flex items-start gap-3">
              <div className="w-10 h-10 rounded-full bg-rose-100 flex items-center justify-center flex-shrink-0">
                <span className="text-rose-700 text-base font-semibold">{g.name[0]}</span>
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <h2 className="text-lg font-semibold text-stone-900">{g.name}</h2>
                  {g.is_vip && <Star className="w-4 h-4 text-amber-400 fill-amber-400" />}
                  {g.rsvp_submitted_at && <span className="text-xs bg-green-50 text-green-700 px-1.5 py-0.5 rounded-full">RSVP'd</span>}
                  {g.checked_in_at && <span className="text-xs bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded-full font-medium flex items-center gap-1"><CheckCircle2 className="w-3 h-3" />Checked in</span>}
                </div>
                <div className="flex items-center gap-3 mt-0.5 text-xs text-stone-400 flex-wrap">
                  {g.phone && <a href={`tel:${g.phone}`} className="flex items-center gap-1 hover:text-stone-600"><Phone className="w-3 h-3" />{g.phone}</a>}
                  {g.email && <span className="flex items-center gap-1"><Mail className="w-3 h-3" />{g.email}</span>}
                  {g.plus_count > 0 && <span className="flex items-center gap-1"><Users className="w-3 h-3" />+{g.plus_count}</span>}
                  {alloc?.rooms && <span className="bg-stone-100 px-1.5 py-0.5 rounded text-stone-600">Room {alloc.rooms.room_number}</span>}
                </div>
              </div>
            </div>
          </SheetHeader>
          ...
          <div className="flex items-center gap-2 mt-3">
            {g.phone && (
              <a href={`https://wa.me/${g.phone.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer">
                <Button size="sm" variant="outline" className="h-7 px-3 text-xs text-emerald-700 border-emerald-200 hover:bg-emerald-50">
                  <MessageCircle className="w-3 h-3 mr-1" /> WhatsApp
                </Button>
              </a>
            )}
            <Button size="sm" variant="outline" className="h-7 px-3 text-xs"
              onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/rsvp/${g.rsvp_token}`); toast.success('RSVP link copied') }}>
              Copy RSVP link
            </Button>
          </div>

          {/* Tab bar */}
          <div className="flex gap-0 mt-4 -mb-4 border-b border-stone-100">
            {TABS.map(t => (
              <button key={t.key} onClick={() => setTab(t.key)}
                className={`flex items-center gap-1.5 px-4 py-2.5 text-xs font-medium border-b-2 transition-colors ${tab === t.key ? 'border-rose-700 text-rose-700' : 'border-transparent text-stone-500 hover:text-stone-700'}`}>
                ...
                return (
                  <div key={ev.id} className="flex items-center gap-3 px-4 py-3 bg-white border border-stone-200 rounded-xl cursor-pointer hover:border-stone-300 transition-colors"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-stone-400">{fmtDate(ev.date)}</p>
                    </div>
